  ...
  <div>
    <el-card>
      <el-form :inline="true" :model="searchForm">
        <el-form-item label="账号"><el-input v-model="searchForm.yonghuzhanghao" placeholder="请输入" clearable /></el-form-item>
        <el-form-item label="姓名"><el-input v-model="searchForm.yonghuxingming" placeholder="请输入" clearable /></el-form-item>
        <el-form-item>
      ...
      <el-table :data="tableData" @selection-change="handleSelectionChange" border stripe>
        ...
        <el-table-column prop="yonghuzhanghao" label="用户账号" />
        <el-table-column prop="yonghuxingming" label="用户姓名" />
    ...
    <el-dialog v-model="dialogVisible" :title="isEdit ? '编辑' : '新增'" width="500px">
      <el-form :model="form" label-width="100px">
        <el-form-item label="用户账号"><el-input v-model="form.yonghuzhanghao" :disabled="isEdit" /></el-form-item>
        <el-form-item label="用户姓名"><el-input v-model="form.yonghuxingming" /></el-form-item>
...
const searchForm = reactive({ yonghuzhanghao: '', yonghuxingming: '' })
const tableData = ref([])
const page = ref(1)
const limit = ref(10)
const total = ref(0)
...
const loadData = async () => {
  const { data: res } = await http.get('/yonghu/page', { params: { ...searchForm, page: page.value, limit: limit.value } })
  if (res.code === 0) { tableData.value = res.data.list; total.value = res.data.total }
}

const resetSearch = () => { Object.assign(searchForm, { yonghuzhanghao: '', yonghuxingming: '' }); loadData() }

const openDialog = (row) => {
  isEdit.value = !!row
  form.value = row ? { ...row } : { xingbie: '男', money: 0, mima: '123456' }
...
const handleBatchDelete = async () => {
  ...
  await ElMessageBox.confirm(`确定删除选中的${selectedIds.value.length}条数据？`, '提示')

  ...
  <div>
    <el-card>
      <el-form :inline="true" :model="searchForm">
        <el-form-item label="分类名称"><el-input v-model="searchForm.shujifenlei" placeholder="请输入" clearable /></el-form-item>
        <el-form-item>
          <el-button type="primary" @click="loadData">查询</el-button>
          <el-button @click="searchForm.shujifenlei = ''; loadData()">重置</el-button>
      ...
      <el-table :data="tableData" @selection-change="sel => selectedIds = sel.map(r=>r.id)" border stripe>
        ...
        <el-table-column prop="shujifenlei" label="书籍分类" />
    ...
    <el-dialog v-model="dialogVisible" :title="isEdit ? '编辑' : '新增'" width="400px">
      <el-form :model="form" label-width="100px">
        <el-form-item label="分类名称"><el-input v-model="form.shujifenlei" /></el-form-item>
      </el-form>
      <template #footer>
...
const searchForm = reactive({ shujifenlei: '' })
...
const loadData = async () => {
  const { data: res } = await http.get('/shujifenlei/page', { params: { ...searchForm, page: page.value, limit: limit.value } })
...
const handleSave = async () => {
  const { data: res } = await http.post(isEdit.value ? '/shujifenlei/update' : '/shujifenlei/save', form.value)
  if (res.code === 0) { ElMessage.success('操作成功'); dialogVisible.value = false; loadData() } else ElMessage.error(res.msg)
}
const handleDelete = async (id) => { await ElMessageBox.confirm('确定删除？'); const { data: res } = await http.post('/shujifenlei/delete', [id]); if (res.code === 0) { ElMessage.success('删除成功'); loadData() } }
const handleBatchDelete = async () => {
  if (!selectedIds.value.length) return ElMessage.warning('请选择数据')
  await ElMessageBox.confirm(`确定删除${selectedIds.value.length}条？`); const { data: res } = await http.post('/shujifenlei/delete', selectedIds.value); if (res.code === 0) { ElMessage.success('删除成功'); loadData() }
}
onMounted(loadData)
</script>
